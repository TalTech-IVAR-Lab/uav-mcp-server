...
"$PYTHON_BIN" "$SCRIPT_DIR/configure_sitl_params.py" \
...
setsid bash -lc \
  "cd '$REPO_ROOT' && export HOST='$HOST' PORT='$PORT' BACKEND_MODE='live' PX4_CONNECTION_STRING='$PX4_CONNECTION_STRING' && scripts/run_demo.sh http live" \
  >"$SERVER_LOG" 2>&1 &
server_pid="$!"
